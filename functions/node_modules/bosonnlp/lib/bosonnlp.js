...
BosonNLP.prototype.rate_limit_status = function(callback) {
    postOptions.path = "/application/rate_limit_status.json";
    // 查询API 频率限制
    sendPost(postOptions, null, callback);
};
BosonNLP.prototype.suggest = function(data, options, callback) {
    if (!callback) {
        callback = options;
        options = {};
    }
    callback = callback || function() {};
    options.top_k = options.top_k || 10;
    postOptions.path = "/suggest/analysis?top_k=" + options.top_k;
    var datas = parseDatas(data);
    sendPost(postOptions, datas, callback);
};
